            ...
            <tbody>
                @foreach ($students as $key => $d)
                    <tr class="computation-container">
                        ...
                        <?php $grade = $d->student->grades->where('classes_subject_id', $data->id)->first(); ?>
                        @if (!empty($data->computations))
                            @foreach ($data->computations as $computation_key => $computation)
                                <?php if (!empty($grade)) {$grades_item = $grade->items->where('computation_id', $computation->id)->first();} ?>
                                <td>
                                    <input value="{{!empty($grades_item->value) ? $grades_item->value: ''}}" name="computations[{{ $computation->id }}]" min="1" max="100" form="form{{$key}}" type="number" class="form-control decimal-input computation-input" computaion-value="{{ $computation->value }}" required>
                                </td>
                            @endforeach
                        @endif
